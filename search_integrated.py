...
LOCAL_LLM_URL = "http://localhost:1234/v1/chat/completions"  # Default LM Studio API endpoint
MODEL_NAME = "phi-3.1-mini-4k-instruct"  # Model name can be arbitrary for local models
...
def query_local_llm(messages, max_tokens=2048, temperature=0.7):
    """
    Query the local LLM (LM Studio) with the given messages.
    ...
    Returns:
        The generated response text or error message if there was an error
    """
    headers = {
    ...
    try:
        response = requests.post(LOCAL_LLM_URL, headers=headers, json=payload, timeout=60)
        ...
        if 'choices' in response.json() and len(response.json()['choices']) > 0:
            ...
            print("Unexpected response format from local LLM:", response.text)
            return "Error: Could not parse response from local LLM"
            
    except requests.exceptions.RequestException as e:
        error_msg = f"Error connecting to local LLM at {LOCAL_LLM_URL}. "
        error_msg += "Make sure LM Studio is running with the API server enabled."
        print(error_msg)
        return f"Error: {error_msg}"

def generate_recommendation_with_llm(user_query, retrieved_papers_content, google_search_results=None):
    """
    Uses the local LLM to generate a recommendation based on the user's query,
...
Ensure your recommendation is grounded in the provided context and directly addresses the user's query."""
    ...
    # Call the local LLM
    response = query_local_llm(messages, max_tokens=2000, temperature=0.7)
    
    if not response or response.startswith("Error:"):
        return "I'm sorry, I couldn't generate a recommendation at this time. Please ensure LM Studio is running with the API server enabled."
